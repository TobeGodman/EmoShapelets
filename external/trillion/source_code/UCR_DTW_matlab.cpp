...
/// data structure that is returned from main
...
/// Main Function
Results UCR_DTW_matlab(vector<double> dataSeries, vector<double> querySeries,  int m, float R)
{
    ...
    // fill in query series with the values from vectors
    for (int itt=0; itt<m; itt++)
    {
        d = querySeries.at(itt);
        ex += d;
        ex2 += d*d;
        q[itt] = d;
    }
    ...
    ex = ex2 = 0;
    bool done = false;
    int it=0, ep=0, k=0, toAccessNext = 0;
    ...
    while(!done)
    {
        ...
        while(ep<EPOCH)
        {
            ...
            if (toAccessNext >= dataSeries.size())
                break;
            buffer[ep] = dataSeries.at(toAccessNext);            
            toAccessNext++;
            ep++;
        }
    ...
    /// return results
    Results results;
    results.bsf = sqrt(bsf);
    results.loc = loc;
    
    return results;
}

/// helper function that prints out content of vector
void printVector(vector<double> VectorIn)
{

    // print out query series
    mexPrintf("*** Vector Content:");
    for (unsigned itt=0; itt<VectorIn.size(); itt++)
        mexPrintf("\n %f", VectorIn.at(itt));        
    mexPrintf("\n");
}
...
int main_debug()
{
    ...
    // initialize vector for query matrix
    vector<double> dataSeries;
    vector<double> querySeries;
    double tempVar;
    int m = 5;
    int i = 0;

    // read query file
    qp = fopen("../Executable/Query.txt","r");
    if( qp == NULL )
        error(2);

    // read query sequence
    while(fscanf(qp,"%lf",&tempVar) != EOF ) // && i < m
    {
        querySeries.push_back(tempVar);
        i++;
    }
    fclose(qp);

    // read data file
    fp = fopen("../Executable/Data.txt","r");
    if( fp == NULL )
        error(2);

    // read data sequence
    while(fscanf(fp,"%lf",&tempVar) != EOF ) // && i < m
    {
        dataSeries.push_back(tempVar);
        i++;
    }
    fclose(fp);
    ...
    Results myResults;
    myResults = UCR_DTW_matlab(dataSeries, querySeries,  querySeries.size(), 0.1);

    cout << "loc: " << myResults.loc << endl;
    cout << "bsf: " << myResults.bsf << endl;
                  ...
                  int nrhs, const mxArray *prhs[])
{
    ...
    double *dataSeries;
    double *querySeries;
    size_t dataSeriesLen;
    size_t querySeriesLen;
    double R;
    ...
    if( mxIsComplex(prhs[2]) ||
        ...
        mexErrMsgIdAndTxt("MyToolbox:arrayProduct:notScalar","Third input (R) must be a scalar.");
    }
    // make sure the first input argument is type double
    if( !mxIsDouble(prhs[0]) || 
         mxIsComplex(prhs[0])) {
        mexErrMsgIdAndTxt("MyToolbox:arrayProduct:notDouble","Input array must be type double.");
    }
    ...
    // check that number of rows in first and second inputs are 1s
    if(mxGetM(prhs[0])!=1 || mxGetM(prhs[1])!=1) {
        mexErrMsgIdAndTxt("MyToolbox:arrayProduct:notRowVector","Input must be a row vector.");
    }

    // create pointers to inputs
    dataSeries = mxGetPr(prhs[0]);
    querySeries = mxGetPr(prhs[1]);
    R = mxGetScalar(prhs[2]);

    // get dimensions of the data and query series
    dataSeriesLen =  mxGetN(prhs[0]);
    querySeriesLen =  mxGetN(prhs[1]);

    // put everything into vectors
    vector<double> dataSeriesVec;
    vector<double> querySeriesVec;

    // push data series
    for (mwSize i=0; i<dataSeriesLen; i++) {
        dataSeriesVec.push_back(dataSeries[i]);
    }
    // push data series
    for (mwSize i=0; i<querySeriesLen; i++) {
        querySeriesVec.push_back(querySeries[i]);
    }

    // call UCR DTW (might need to change later)
    Results myResults;    
    // if (dataSeriesVec.size()<querySeriesVec.size()) {
    //     // myResults.bsf = 50;
    //     // myResults.loc = -2;
    //     myResults = UCR_DTW_matlab(querySeriesVec, dataSeriesVec,  dataSeriesVec.size(), R);
    // }        
    // else {
    //     // myResults.bsf = 50;
    //     // myResults.loc = -2;
    //     myResults = UCR_DTW_matlab(dataSeriesVec, querySeriesVec,  querySeriesVec.size(), R);
    // }
    ...
    // clear vectors
    dataSeriesVec.clear();
    querySeriesVec.clear();

    // update bsf and loc
    bsf = myResults.bsf;
    loc = myResults.loc;
    
    // create pointers to outputs    
    plhs[0] = mxCreateDoubleScalar(bsf);
    plhs[1] = mxCreateDoubleScalar(loc);
